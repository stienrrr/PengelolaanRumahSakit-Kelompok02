@extends('cms.layout.main-cms', ['title' => 'Add Prescription'])

@section('content')
	<div class="main-content">
		<!--breadcrumb-->
		<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
			<div class="h4">Add Prescription</div>
		</div>
		<!--end breadcrumb-->
		...
		<div class="col-8 col-xl-8 mx-auto">
			<div class="card">
				<div class="card-body p-4">
					<form class="row g-3" action="{{ route('prescriptions.store') }}" method="POST">
						@csrf
						<div class="col-md-12">
							...
							<select class="form-select mb-3" aria-label="Default select example" id="medical_record_id" name="medical_record_id">
								...
								@foreach ($medicalRecords as $md)
									<option value="{{ $md->id }}">{{ $md->patient->name }} - {{ $md->doctor->name }} - {{ $md->registration->registration_code }}</option>
								@endforeach
							</select>
						</div>
